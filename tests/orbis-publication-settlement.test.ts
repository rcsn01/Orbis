...
import { createPublicationSettlement, type ScanLifecycleFocusPort, type SettlementResult } from "../src/main/publication-settlement"
import type { CompletedOutcome, ScanRunContext, UnchangedOutcome } from "../src/main/scan-run-lifecycle"
import { FullScanResumeStore, type FullScanResumeDescriptor } from "../src/main/full-scan-resume"
import { createEmptyCatalog, LocationCatalogStore, type PendingScanRecord, type ResolvedLocationTarget } from "../src/main/location-catalog"
...
interface SettlementFixture {
  ...
  readonly coverage: ReturnType<typeof createCoveragePublicationAccess>
  readonly settle: (run: ScanRunContext, outcome: CompletedOutcome | UnchangedOutcome) => Promise<SettlementResult>
...
async function fixture(name: string): Promise<SettlementFixture> {
  const directory = await mkdtemp(join(tmpdir(), `orbis-settlement-${name}-`))
  ...
  const settlement = createPublicationSettlement({
  ...
  return {
    directory, indexes, target: identity.target, identity, catalog, coverage, settle: (run, outcome) => settlement.settle(run, outcome), events,
...
async function scanResultFor(value: SettlementFixture, generation: number, paths: { readonly partialPath: string; readonly publishedPath: string }): Promise<ScanResult> {
...
async function publishOnce(value: SettlementFixture): Promise<string> {
...
function runFor(value: SettlementFixture, generation: number, basePublicationId: string | null): ScanRunContext {
  const owner = value.catalog.current.locations[0]!
...
function pendingRecordFor(value: SettlementFixture, run: ScanRunContext): PendingScanRecord {
  return {
...
function unchangedOutcome(basePublicationId: string): UnchangedOutcome {
  return { kind: "unchanged", journal: { uuid: "volume", eventId: "42" }, totals: UNUSED_TOTALS, basePublicationId }
}
...
describe("publication settlement", () => {
  it("publishes a completed run by installing the candidate before any best-effort retirement", async () => {
    ...
    try {
      ...
      await value.catalog.beginScan(pendingRecordFor(value, run), undefined)
      ...
      const publication = await value.settle(run, { kind: "completed", result })
  ...
  it("keeps a stale base publication from publishing at all", async () => {
    ...
    try {
      ...
      value.events.length = 0
      const publication = await value.settle(run, {
  ...
  it("retires Resume before advancing an unchanged publication", async () => {
    ...
    try {
      ...
      await value.catalog.beginScan(pendingRecordFor(value, run), undefined)
      value.events.length = 0
      const publication = await value.settle(run, unchangedOutcome(installedScanId))
      expect(publication).toMatchObject({ kind: "published", totals: UNUSED_TOTALS, warnings: [] })
  ...
  it("refuses an unchanged publication when another scan owns the Resume descriptor", async () => {
    ...
    try {
      ...
      value.events.length = 0
      await expect(value.settle(run, unchangedOutcome(installedScanId))).rejects.toThrow("Another Orbis scan owns the Resume descriptor")
      expect(value.events).toEqual([])
  ...
  it("keeps an advanced unchanged publication when run-file cleanup fails", async () => {
    ...
    try {
      ...
      const failingSettlement = createPublicationSettlement({
      ...
      const publication = await failingSettlement.settle(run, unchangedOutcome(installedScanId))
      expect(publication).toMatchObject({ kind: "published" })
  ...
  it("reports an unchanged publication against a missing base as stale", async () => {
    ...
    try {
      ...
      const run = runFor(value, 1, baseId)
      const publication = await value.settle(run, unchangedOutcome(baseId))
      expect(publication).toEqual({ kind: "stale", reason: "stale-base-publication", candidateDisposition: "catalog-owned" })
    } finally { await value.coverage.close() }
  })

  it("only ever rejects an unchanged outcome for base-publication reasons", async () => {
    const value = await fixture("unchanged-invariant")
    try {
      const installedScanId = await publishOnce(value)
      // The former ScanStaleBasePublicationResult type guaranteed that an
      // unchanged outcome could not return `stale-install`; the guarantee is
      // now behavioral. Every way an unchanged settlement can go stale resolves
      // to the base-publication reason: a missing installation ...
      const missing = await value.settle(runFor(value, 2, randomUUID()), unchangedOutcome(randomUUID()))
      expect(missing).toEqual({ kind: "stale", reason: "stale-base-publication", candidateDisposition: "catalog-owned" })
      // ... an installed publication whose id does not match the outcome ...
      const mismatchedId = await value.settle(runFor(value, 3, installedScanId), unchangedOutcome(randomUUID()))
      expect(mismatchedId).toEqual({ kind: "stale", reason: "stale-base-publication", candidateDisposition: "catalog-owned" })
      // ... and a run targeted outside the installed publication.
      const outside = { ...runFor(value, 4, installedScanId), target: join(value.directory, "elsewhere") }
      const wrongTarget = await value.settle(outside, unchangedOutcome(installedScanId))
      expect(wrongTarget).toEqual({ kind: "stale", reason: "stale-base-publication", candidateDisposition: "catalog-owned" })
      // No stale path touched the catalog, Resume, or the pending record.
      expect(value.events).toEqual([])
      expect(value.catalog.current.pendingScan).toBeNull()
    } finally { await value.coverage.close() }
  })
