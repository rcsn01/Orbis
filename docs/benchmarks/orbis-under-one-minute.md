...
Parallelize native directory traversal with a bounded worker queue to leave time for index construction. Then return compact node batches with parent relationships and bottom-up totals so JavaScript can bulk-build the published SQLite schema without per-directory construction tasks or recursive ancestor updates.
